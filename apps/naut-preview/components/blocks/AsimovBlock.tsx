...
export default function AsimovBlock(props: Props) {
  return (
    <div className="relative isolate pt-24 sm:pt-32">
      ...
      >
        <div
          className="ml-[max(50%,38rem)] aspect-[1313/771] w-[82.0625rem] bg-gradient-to-tr from-color2 to-color2"
          style={{
            clipPath:
      ...
      >
        <div
          className="ml-[-22rem] aspect-[1313/771] w-[82.0625rem] flex-none origin-top-right rotate-[30deg] bg-gradient-to-tr from-color2 to-color2 xl:ml-0 xl:mr-[calc(50%-12rem)]"
          style={{
            clipPath:
              "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
          }}
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto mt-16 grid max-w-2xl grid-cols-1 grid-rows-1 gap-8 text-sm leading-6 text-title sm:mt-20 sm:grid-cols-2 xl:mx-0 xl:max-w-none xl:grid-flow-col xl:grid-cols-4">
          <motion.figure initial={{opacity: 0, y: 20}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.2}} viewport={{amount: 0.9, once: true}} className="col-span-2 hidden sm:block sm:rounded-2xl sm:bg-bg1/50 bg-blur sm:shadow-lg sm:ring-1 sm:ring-text/5 xl:col-start-2 xl:row-end-1 transition-none">
          ...
          <div className="space-y-8 xl:contents xl:space-y-0">
            <div className="xl:row-span-2 space-y-8">
              {props.testimonials1.map((testimonial, testimonialIndex) => (
                <motion.figure
                  ...
                  className="rounded-2xl bg-bg1/50 p-6 bg-blur shadow-lg ring-1 ring-text/5 transition-none"
                  initial={{opacity: 0, y: 20}}
                  whileInView={{opacity: 1, y: 0}}
                  transition={{duration: 0.2}}
                  viewport={{amount: 0.9, once: true}}
            ...
            <div className="xl:row-start-1 space-y-8">
              {props.testimonials2.map((testimonial, testimonialIndex) => (
                <motion.figure
                  key={testimonial.name}
                  className="rounded-2xl bg-bg1/50 p-6 shadow-lg ring-1 ring-text/5 transition-none bg-blur"
                  initial={{opacity: 0, y: 20}}
                  whileInView={{opacity: 1, y: 0}}
                  transition={{duration: 0.2}}
                  viewport={{amount: 0.9, once: true}}
          ...
          <div className="space-y-8 xl:contents xl:space-y-0">
            ...
            >
              {props.testimonials3.map((testimonial, testimonialIndex) => (
                <motion.figure
                  key={testimonial.name}
                  className="rounded-2xl bg-bg1/50 bg-blur p-6 shadow-lg ring-1 ring-text/5 transition-none"
                  initial={{opacity: 0, y: 20}}
                  whileInView={{opacity: 1, y: 0}}
                  transition={{duration: 0.2}}
                  viewport={{amount: 0.9, once: true}}
            ...
            >
              {props.testimonials4.map((testimonial, testimonialIndex) => (
                <motion.figure
                  key={testimonial.name}
                  className="rounded-2xl p-6 shadow-lg ring-1 ring-text/5 transition-none bg-bg1/50 bg-blur"
                  initial={{opacity: 0, y: 20}}
                  whileInView={{opacity: 1, y: 0}}
                  transition={{duration: 0.2}}
                  viewport={{amount: 0.9, once: true}}
