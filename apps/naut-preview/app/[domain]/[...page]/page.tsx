...
export async function generateMetadata(
  ...
  if (pageParams.page?.[0] == "cms") {
    if (pageParams.page?.[1] == "entry") {
      ...
      const cmsTypeData:any = await db.select().from(cmsType).where(and(eq(cmsType.uid, cmsTypeUid), eq(cmsType.siteDomain, domain)));
      const siteData:any = await db.select().from(site).where(eq(site.domain, domain));
...
export default async function Page({ params }: Props) {
  ...
  if (pageParams.page?.[0] == "cms") {
    if (pageParams.page?.[1] == "entry") {
      ...
      const cmsTypeUid = pageParams.page?.[2];
      const pageNumber = pageParams.page?.[3] || 1;
      const cmsTypeData = await db.select().from(cmsType).where(and(eq(cmsType.uid, cmsTypeUid), eq(cmsType.siteDomain, domain)));
