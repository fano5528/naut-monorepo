    ...
    <div className="w-complete sm:w-complete-sm mt-16 sm:mt-24 mx-auto">
      ...
      <div className="h-auto lg:h-[600px] grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] xl:grid-cols-2 gap-12 lg:gap-6 lg:grid-rows-3 max-w-7xl">
        ...
        {fullPosts.map((post, index) => (
          ...
          >
            <Link edit={props.edit} href={`/cms/entry/${post.id}`} className="h-full lg:shrink-0">
              <Image
            ...
            <div className="flex flex-col justify-start">
              ...
              <Link edit={props.edit} href={`/cms/entry/${post.id}`} className="text-lg font-bold text-color1 group-hover:text-color2 transition-colors mt-1 leading-normal">
                {post.title}
              </Link>
            </div>
          </div>
