...
export default async function KingBlock(props: {
  title: string;
  subtitle: string;
  ctaText: string;
  typeUid: string;
  ...
  const postsInDb = await db.select().from(cmsEntry).where(eq(cmsEntry.cmsTypeId, blogType[0].id)).orderBy(desc(cmsEntry.publishedAt)).limit(4);
